...
const CaseDetailPage = () => {
  ...
  const { singleComplaint, fetchCaseDetail, loading } = useComplaintStore();

  useEffect(() => {
    if (!caseId) return;

    // fetch only if missing or not the same complaint
    if (!singleComplaint || singleComplaint.caseId !== caseId) {
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-4">
              <h1 className="text-slate-900 dark:text-white text-4xl font-black leading-tight tracking-tight">
                Complaint #{singleComplaint.caseId}
              </h1>
              <span className="px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 text-xs font-bold uppercase tracking-wider">
                {ComplaintStatus(singleComplaint.status)}
              </span>
              <span className="px-3 py-1 rounded-full bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs font-bold uppercase tracking-wider">
                {singleComplaint.priority}
              </span>
            </div>
            <p className="text-slate-500 dark:text-slate-400 text-base font-normal">
              Submitted on{" "}
              {formatDate(singleComplaint.createdAt).formattedDate()} |{" "}
              <span className="font-semibold capitalize">
                {singleComplaint.reportingMode} Reporter
              </span>
            </p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => generateComplaintPdf(singleComplaint)}
              className="flex items-center justify-center rounded-lg h-10 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm font-bold shadow-sm"
            >
          ...
          <div className="col-span-full md:col-span-8 flex flex-col gap-8">
            ...
            <div className="border-b border-slate-200 dark:border-slate-800">
              <div className="flex gap-8">
                ...
                >
                  ...
                  Evidence ({singleComplaint.evidence.length})
                </a>
                <a
                  className="flex items-center border-b-2 border-transparent text-slate-500 dark:text-slate-400 pb-3 pt-2 font-medium text-sm hover:text-slate-700"
                  href="#"
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
              <div className="p-6">
                ...
                {singleComplaint.evidence.length > 0 && (
                  <>
                    <h3 className="text-slate-900 dark:text-white font-bold mt-8 mb-4">
                      Evidence &amp; Attachments
                    </h3>
                    <div className="grid grid-cols-2 md:grid-cols-1 lg:grid-cols-2 gap-4">
                      {singleComplaint.evidence.map((file) => (
                        <div
                          key={file.url}
                          className="flex flex-col sm:flex-row items-center p-3 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800/50"
                        >
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-500">
                              ...
                              {formatDate(file.uploadedAt).formattedDate()}
                            </p>
                          </div>
              ...
              <div className="p-6 h-80 overflow-y-auto space-y-6 bg-slate-50/30 dark:bg-slate-900/50">
                ...
                <div className="flex flex-col items-start max-w-[80%]">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-4 rounded-xl rounded-tl-none border border-slate-200 dark:border-slate-700 text-sm text-slate-700 dark:text-slate-300 shadow-sm">
                    Hello, I've uploaded the documents. Is there anything else
                    you need to verify this claim?
                  </div>
                </div>
                {/* <!-- Message 2 (Admin) --> */}
                <div className="flex flex-col items-end ml-auto max-w-[80%]">
                  ...
                  <div className="bg-primary text-white p-4 rounded-xl rounded-tr-none shadow-md text-sm">
                    Thank you for your submission. We have received the files
                    and assigned an investigator. Can you clarify if you know
                    who authorized the final payment on project #APX-004?
                  </div>
                </div>
                {/* <!-- Message 3 --> */}
                <div className="flex flex-col items-start max-w-[80%]">
